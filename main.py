            ...
            if self.have_active_piece:
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_DOWN:
                        self.piece.move_down(self.game_grid)
                
                    # Esquerda
                    if event.key == pygame.K_LEFT:
                        self.piece.move_left(self.game_grid)

                    # Direita
                    if event.key == pygame.K_RIGHT:
                        self.piece.move_right(self.game_grid)
    ...
    def _draw(self) -> None:
        ...
        for y, row in enumerate(self.game_grid.matrix):
            for x, tile_type in enumerate(row):
                ...
                if tile_type != 0:
                    pygame.draw.rect(self.screen, COLORS[self.piece.type], tile)
